# Kymera Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the kymera simulator library + lab layer per the approved spec (`docs/specs/2026-06-11-kymera-design.md`), with bit-parity against zymera v0, and ship webpage-style docs with an env tutorial.

**Architecture:** Component-composition simulator (`kymera`: env/worldgen/dynamics/comms/obs/missions/metrics/rollout/viz) + `kymera.lab` (config/runio/eval/nets/ppo). Parallel build — `../zymera_env/` is **never modified**; parity via golden files dumped read-only from v0.
...
Kymera `GossipChannel(delay=1, dropout=0).deliver`: ring buffer holds previous
...
**Files:** Create `pyproject.toml`, `kymera/__init__.py` (stub), `README.md` (stub), `.gitignore`, `tests/conftest.py`.

- [ ] `pyproject.toml`: name `kymera` 0.1.0, `requires-python>=3.10`; core deps `jax>=0.4, jaxlib>=0.4, chex>=0.1, numpy`; extras `viz=[matplotlib,pillow]`, `lab=[equinox>=0.11,optax>=0.1,msgspec>=0.18,pyyaml>=6.0]`, `dev=[pytest>=8.0]`; `packages.find include=["kymera*"]`.
...
### Task 2: Core contract — `kymera/env.py` (schema half) + `kymera/rollout.py`

**Files:** Create `kymera/env.py` (ActionId, ACTION_DELTAS, Body, World, Env base, registry, `_info_dict`), `kymera/rollout.py`, `tests/test_state.py`, `tests/test_rollout.py`.
...
### Task 3: `kymera/metrics.py` (+ StepCtx)

**Files:** Create `kymera/metrics.py`, `tests/test_metrics.py`.
...
### Task 4: `kymera/worldgen.py`

**Files:** Create `kymera/worldgen.py`, `tests/test_worldgen.py`.
...
### Task 5: `kymera/dynamics.py`

**Files:** Create `kymera/dynamics.py`, `tests/test_dynamics.py`.
...
### Task 6: `kymera/comms.py`

**Files:** Create `kymera/comms.py`, `tests/test_comms.py`.
...
### Task 7: `kymera/obs.py`

**Files:** Create `kymera/obs.py`, `tests/test_obs.py`.
...
### Task 8: `kymera/missions.py` + `kymera/missions_terms.py`

**Files:** Create `kymera/missions.py`, `kymera/missions_terms.py` (flat module; spec allows growth into a package later), `tests/test_missions.py`.
...
**Files:** Modify `kymera/env.py`; create `tests/test_env.py`, `tests/test_parity.py`.
...
**Files:** Create `kymera/viz/{__init__.py, render.py, iso.py, report.py, live.py, annotate.py}`, `tests/test_viz.py`.
...
**Files:** Create `kymera/lab/{__init__.py, config.py, runio.py, eval.py, nets.py, ppo.py}`, `tests/test_lab.py`.
...
- [ ] `index.html`: what kymera is, install, 5-line quick start, public-API table, component map, doctrine list, links.
...
- [ ] Verify `import kymera` pulls no matplotlib/equinox (headless core check).
