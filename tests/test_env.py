...
import zymera
from zymera.comms import DiskTopology, GossipChannel, NullChannel
from zymera.missions import FixedAssignment, GroupedMission, Mission, RewardTerm


def _tiny(**kw):
    return zymera.make("comm-coverage", grid=6, n_agents=3, comm_r=2,
                       spawn_radius=1, **kw)
...
def test_recipes_registered():
    assert {"empty", "comm-coverage"} <= set(zymera.list_envs())
...
def test_belief_obs_requires_channel():
    from zymera.obs import GridObs
    with pytest.raises(ValueError, match="post-gossip belief"):
        zymera.GridEnv(grid_h=6, grid_w=6, n_agents=2,
...
def test_bad_collision_string_raises():
    with pytest.raises(ValueError, match="collision"):
        zymera.make("comm-coverage", collision="masked")


def test_unknown_term_shorthand_raises():
    with pytest.raises(ValueError, match="unknown term"):
        zymera.make("comm-coverage", terms=[("nonsense", 1.0)])
...
def test_spec_roundtrip(key):
    env = zymera.make("comm-coverage", grid=8, n_agents=3, comm_r=2, spawn_radius=1)
    env2 = zymera.make_from(env.spec())
...
def test_direct_construction_has_no_spec():
    env = zymera.GridEnv(grid_h=6, grid_w=6, n_agents=2)
...
def test_mission_structure_stable_for_all_registered_envs(key):
    ...
    for name in zymera.list_envs():
        env = zymera.make(name) if name != "comm-coverage" else _tiny()
        _, s0 = env.reset(key)
...
def test_rollout_jit_vmap(key):
    env = _tiny()
    f = jax.jit(lambda k: zymera.rollout(env, zymera.random_policy, 5, k))
    traj = f(key)
    assert traj["obs"].shape == (6, 3, 5, 6, 6)
    batch = jax.vmap(lambda k: zymera.rollout(env, zymera.random_policy, 5, k))(
...
def test_rollout_lean_drops_channel(key):
    env = _tiny()
    lean = zymera.rollout(env, zymera.random_policy, 4, key)
    full = zymera.rollout(env, zymera.random_policy, 4, key, keep="all")
    assert lean["world"].channel == ()
    assert full["world"].channel.shared.shape == (5, 3, 6, 6)


def test_rollout_collect_reward_terms(key):
    traj = zymera.rollout(_tiny(), zymera.random_policy, 4, key,
...
def test_block_collision_never_shares_cell(key):
    ...
    traj = zymera.rollout(env, zymera.random_policy, 15, key, keep="all")
...
def test_max_steps_done(key):
    env = zymera.make("comm-coverage", grid=6, n_agents=3, comm_r=2,
...
def test_grouped_mission_routes_reward(key):
    from zymera import missions_terms as mt
